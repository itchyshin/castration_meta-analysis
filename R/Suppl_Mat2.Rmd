...
phy_sex <- ggplot(data = cdat, aes(x = yi, y = Phylo_Sex)) +
  ...
  geom_point(aes(fill = Species), size = 1, shape = 21) + 
  xlim(-0.8, 1.6) +
  theme_bw() +
...
dat_long$Comp_type_Sex <- factor(dat_long$Comp_type_Sex, 
...
test<- orchard_plot(mod_comp_sex, mod = "Comp_type_Sex", xlab = "log response ratio (lnRR)", group = "Study", data = dat_long, cb = F, angle = 45)

# this does not work
#test + coord_flip()

# trying to draw paried figures
