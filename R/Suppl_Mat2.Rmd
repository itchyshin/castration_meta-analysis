...
groundhog.library("emmeans", '2022-01-01')
...
# # some addition
# 
# rob1 <- robust(mod, cluster = Study, adjust=FALSE, clubSandwich=FALSE)
# rob2 <- robust(mod, cluster = Study, adjust=TRUE, clubSandwich=FALSE)
# #rob3 <- robust(mod, cluster = Study, adjust=TRUE, clubSandwich=TRUE, verbose=TRUE)
# 
# rob1
# rob2
# #rob3
...
## Sex-wise species-level effect
```{r}
# creating mod_table- results from the model

results <- mod_results(mod2, group = "Study", data = dat)[[1]]
...
phy_sex <- ggplot(data = ddat, aes(x = yi, y = Species_Latin)) +
  ...
  theme(legend.position= c(0.95, 0.9), legend.justification = c(1, 0)) +
...
```{r}
# there is probably no meaningful effect - not sure whether it should go into the main text
...
bubble_plot(mod_mat, mod = "Maturity_at_treatment_ordinal", data = dat, group = "Study",)
```


### Control_treatment (sham)

```{r}
# TODO - we need to udpdate

mod_sham <- mod_func(formula = ~ Control_treatment-1)
summary(mod_sham)
# contrast
mod_sham1 <- mod_func(formula = ~ Control_treatment)
summary(mod_sham1)
r2_ml(mod_sham1)
# orchard plot
orchard_plot(mod_sham1, mod = "Control_treatment", xlab = "log response ratio (lnRR)", group = "Study", data = dat, cb = F)
...
# probably this is most appoprate to report??
# equal averaging
...
# # some addition
# 
# rrob1 <- robust(rmod, cluster = Study, adjust=FALSE, clubSandwich=FALSE)
# rrob2 <- robust(rmod, cluster = Study, adjust=TRUE, clubSandwich=FALSE)
```
...
#rrob_sex1 <- robust(rmod_sex, cluster = Study, adjust=FALSE, clubSandwich=FALSE)
#rrob_sex2 <- robust(rmod_sex, cluster = Study, adjust=TRUE, clubSandwich=FALSE)
...
orchard_plot(mod_comp_sex, mod = "Comp_type_Sex1", xlab = "absolute log response ratio (lnRR)", group = "Study", data = dat_long, cb = T, angle = 45)
...
# absolute
p1 <- orchard_plot(abs_mod_comp_sex, mod = "Comp_type_Sex1", xlab = "absolute log response ratio (lnRR)", group = "Study", data = dat_long, cb = T, angle = 45)

# Pair figures
# female
female_dat_log <- dat_long %>% filter(Sex == "Female")

p2 <- ggplot(female_dat_log, aes(x = Comp_type, y = abs_yi)) + 
...
p3 <- orchard_plot(f_mod_long, xlab = "absolute log response ratio (lnRR)", group = "Study", data = f_dat_log, cb = F, angle = 45) + 
  scale_fill_manual(values =  "#999933") +
  scale_colour_manual(values =  "#999933") +
  scale_x_discrete(labels = "Female sterlized/\nMale normal vs. \nFemale normal/\nMale normal") +
  ylim(-0.9, 0.5)

# pair figure male  
male_dat_log <- dat_long %>% filter(Sex == "Male")

p4 <- ggplot(male_dat_log, aes(x =Comp_type, y = abs_yi)) + 
  geom_point(aes(size = sqrt(1/abs_vi), col = Comp_type), alpha = 0.5) +
  geom_line(aes(group = Effect_ID), alpha = 0.5) + 
  labs(y = "absolute log response ratio (lnRR)", x = "", size = "Precision (1/SE)") + 
...
p5 <- orchard_plot(m_mod_long, xlab = "absolute log response ratio (lnRR)", group = "Study", data = m_dat_log, cb = F, angle = 45) + 
   scale_fill_manual(values = "#332288") +
  scale_colour_manual(values = "#332288") +
scale_x_discrete(labels = "Male sterlized/\nFemale normal vs. \nMale normal/\nFemale normal") +
  ylim(-0.9, 0.5)

patch <- p1|(p2/p4)|(p3/p5) + plot_layout()
...
#robust(mod_comp2b, cluster = sdat_long$Shared_control)
